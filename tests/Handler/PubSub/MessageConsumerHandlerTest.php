...
{   
    ...
    public function testPublishPullMessage(): void
    {
        ...
        $messageConsumer = new MessageConsumerHandler();
        $response = $messageConsumer->pullConsumer( self::SUBCRIPTION, self::TOPIC, $stub, self::$pubSubService );
        ...
        $this->assertJsonStringEqualsJsonString($response->getContent(), json_encode([
            'code' => Response::HTTP_OK,
            'message' => 'Pull message for subscription \''. self::SUBCRIPTION .'\' was executed successfully.',
            'status' => 'HTTP_OK',
        ]));
    }
    ...
    /**
     * test if a pull message is republished if it encountered an error
     * @todo make test
     * 
     * @since 1.3.0
     * @version 1.3.0
